...
from db import user_exists, add_user, check_password, build_db, add_score_to_user, get_person_with_highest_score, get_user_score_and_position
import hashlib
...
class Client_communication:
    ...
    def handle_msg(self, client_socket):
        ...
        if cmd == "07":
            position2, username2, score = get_user_score_and_position(data_received)
            client_socket.send((username2 + " " + str(score) + " " + position2).encode())
